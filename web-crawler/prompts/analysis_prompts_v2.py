"""
供应链分析报告 AI 提示词 V2.2

核心思路：给方向、不给死框架
- 友商范围要完整（光电模块+连接器+电源）
- 原材料要覆盖（金属+塑料）
- 但不强制每个都写，有料就说、没料就跳过
- 重点是"有洞见"，不是"填满格子"
"""

# 大宗物料品类
MATERIAL_CATEGORIES = {
    "金属类": ["铜", "镍", "锡", "锌", "金", "铝", "银"],
    "塑料类": ["PVC", "PA66", "PBT", "LDPE", "ABS", "PP", "PET", "PC"]
}

# 完整的友商列表（三大领域）
COMPETITORS = {
    "光电模块": ["Credo", "旭创科技", "新易盛", "天孚通信", "光迅科技", "Finisar"],
    "连接器": ["安费诺", "莫仕", "TE", "中航光电", "得意精密", "意华股份", "金信诺", "华丰科技"],
    "电源": ["奥海科技", "航嘉", "赛尔康", "台达电子"]
}

# System Prompt
ANALYSIS_SYSTEM_PROMPT = """你是立讯技术的战略分析顾问。

**你的风格**：
- 说人话，给干货
- 有洞见，不套话
- 有料就深挖，没料就跳过

**分析主体**：立讯技术（非立讯精密集团）

**三大业务领域**：
- 💡 光电模块
- 🔌 连接器
- ⚡ 电源"""


def get_supply_chain_analysis_prompt(
    company_name: str,
    today: str,
    competitors: list,
    upstream: list,
    downstream: list,
    news_summary: str,
    news_count: int,
    commodity_summary: str = ""
) -> str:
    """生成供应链分析报告的提示词"""
    
    return f"""# 分析任务
为**立讯技术**生成一份竞争格局分析报告。

**日期**：{today}
**新闻数量**：{news_count}条

# 立讯技术画像

**三大业务领域**：
- 💡 **光电模块**：光模块、光纤连接器
- 🔌 **连接器**：Type-C、BTB、FPC、板对板
- ⚡ **电源**：充电器、适配器、无线充电

**主要客户**：苹果、华为、Meta、小米、汽车客户

**友商/竞争对手**（按领域）：
- 💡 光电模块：Credo、旭创科技、新易盛、天孚通信、光迅科技、Finisar
- 🔌 连接器：安费诺、莫仕、TE、中航光电、得意精密、意华股份、金信诺、华丰科技
- ⚡ 电源：奥海科技、航嘉、赛尔康、台达电子

**原材料关注**：
- 金属：铜、镍、锡、锌、铝（连接器/线材成本）
- 塑料：ABS、PP、PA66、PVC、PBT、PC（外壳/护套成本）

# 实时新闻（{news_count}条）
{news_summary if news_summary else '⚠️ 当前时段未抓取到相关新闻'}

# 大宗商品价格
{commodity_summary if commodity_summary else '⚠️ 价格数据暂未获取'}

---

# 写作原则

1. **有料就说，没料就跳过** — 不要为了填表而硬扯不相关的新闻
2. **先观点后数据** — 每个章节先说结论，再用表格/数据支撑
3. **禁止套话** — "加强研发"、"密切关注"、"短期承压中期向好" 这类话不要写
4. **引用来源** — 关键结论附上新闻链接

---

# 报告结构

## 一、本周核心发现

用3-5个要点概括最重要的发现，格式：
- ✅/⚠️/🔴 **[一句话结论]**：具体说明 — [来源](链接)

重点关注：
- 客户订单/战略变化
- 友商重大动作
- 原材料价格异动
- 关税/政策风险

---

## 二、客户动态

分析苹果、华为、Meta、小米等客户的最新动向，重点回答：
- 有什么新订单或产品计划？
- 对立讯技术哪个业务线有影响？

*没有相关新闻就简短说明"本周客户面平稳"，不要硬凑。*

---

## 三、友商竞争分析

按**三大业务领域**分析友商动态：

### 💡 光电模块
分析 Credo、旭创科技、新易盛、天孚通信、光迅科技、Finisar 等的动态。

### 🔌 连接器
分析 安费诺、莫仕、TE、中航光电、得意精密、意华股份、金信诺、华丰科技 等的动态。

### ⚡ 电源
分析 奥海科技、航嘉、赛尔康、台达电子 等的动态。

*每个领域：有新闻就分析，没新闻就写"本周暂无重大动态"。不要把没新闻的友商硬塞进表格。*

### 竞争力雷达图（可选）
如果有足够信息支撑，可以给出竞争力对比。

---

## 四、原材料行情

分析原材料价格对成本的影响，覆盖：
- **金属**：铜、镍、锡、锌、铝
- **塑料**：ABS、PP、PA66、PVC、PBT、PC

重点说明：
- 哪些原材料本周有明显涨跌？
- 对立讯哪个业务线成本影响最大？

*有价格数据就分析，没有就跳过。*

---

## 五、关税/政策影响

如有相关新闻则分析，无则简短说明"本周无重大政策变化"。

---

## 六、SWOT分析

基于本周信息，列出关键的优势/劣势/机会/威胁。
**每条必须有具体依据**，不要写空泛的话。

---

## 七、行动建议

给出**具体可执行**的建议，格式：
- 针对什么问题
- 建议什么动作
- 由谁负责
- 预期效果

**禁止写**："加强管理"、"持续优化"、"密切关注" 等套话。

---

# 格式规范

- 新闻链接：[标题](url)
- 没有数据的地方写"—"或跳过，不要编造
- 报告长度：根据内容多少灵活调整，有料多写、没料少写
"""


def precheck_news_quality(news_list: list) -> dict:
    """预检新闻质量"""
    customer_keywords = ["苹果", "Apple", "华为", "Huawei", "Meta", "iPhone", "iPad", "小米", "OPPO", "vivo"]
    
    # 完整的友商关键词（三大领域）
    competitor_keywords = [
        # 光电模块
        "Credo", "旭创", "中际旭创", "新易盛", "天孚", "光迅", "Finisar", "光模块",
        # 连接器
        "安费诺", "莫仕", "Molex", "TE", "中航光电", "得意精密", "意华", "金信诺", "华丰科技",
        # 电源
        "奥海", "航嘉", "赛尔康", "台达",
        # 通用
        "工业富联", "富士康", "比亚迪电子", "歌尔", "蓝思", "鹏鼎", "东山精密", "领益"
    ]
    
    tariff_keywords = ["关税", "加征", "贸易战", "出口管制", "制裁", "301"]
    
    # 产品线关键词
    product_keywords = {
        "optical": ["光模块", "光纤", "光通信", "800G", "400G", "CPO"],
        "connector": ["连接器", "Type-C", "BTB", "FPC", "接插件", "板对板"],
        "power": ["电源", "充电器", "适配器", "无线充", "快充", "PD", "氮化镓"]
    }
    
    # 原材料关键词
    material_keywords = {
        "metal": ["铜", "镍", "锡", "锌", "铝", "金", "银", "铜价", "镍价"],
        "plastic": ["塑料", "ABS", "PP", "PA66", "PVC", "PBT", "PC", "LDPE", "PET", "工程塑料"]
    }
    
    result = {
        "total_count": len(news_list),
        "has_customer_news": False,
        "has_competitor_news": False,
        "has_tariff_news": False,
        "has_material_news": False,
        "has_metal_news": False,
        "has_plastic_news": False,
        "quality_score": 0,
        "suggestions": [],
        "category_counts": {
            "customer": 0,
            "competitor": 0,
            "optical": 0,
            "connector": 0,
            "power": 0,
            "metal": 0,
            "plastic": 0,
            "tariff": 0
        }
    }
    
    for news in news_list:
        text = news.get("title", "") + news.get("content", "")[:300]
        
        if any(kw in text for kw in customer_keywords):
            result["has_customer_news"] = True
            result["category_counts"]["customer"] += 1
            
        if any(kw in text for kw in competitor_keywords):
            result["has_competitor_news"] = True
            result["category_counts"]["competitor"] += 1
            
        if any(kw in text for kw in tariff_keywords):
            result["has_tariff_news"] = True
            result["category_counts"]["tariff"] += 1
            
        # 产品线
        for key, keywords in product_keywords.items():
            if any(kw in text for kw in keywords):
                result["category_counts"][key] += 1
                
        # 原材料
        if any(kw in text for kw in material_keywords["metal"]):
            result["has_metal_news"] = True
            result["has_material_news"] = True
            result["category_counts"]["metal"] += 1
            
        if any(kw in text for kw in material_keywords["plastic"]):
            result["has_plastic_news"] = True
            result["has_material_news"] = True
            result["category_counts"]["plastic"] += 1
    
    # 评分（简化）
    score = min(len(news_list) * 3, 30)  # 基础分
    if result["has_customer_news"]: score += 20
    if result["has_competitor_news"]: score += 20
    if result["has_material_news"]: score += 15
    if result["has_tariff_news"]: score += 10
    if result["category_counts"]["optical"] > 0: score += 5
    if result["category_counts"]["connector"] > 0: score += 5
    if result["category_counts"]["power"] > 0: score += 5
    result["quality_score"] = min(score, 100)
    
    # 建议（精简）
    if not result["has_customer_news"]:
        result["suggestions"].append("缺少客户（苹果/华为/Meta）新闻")
    if not result["has_competitor_news"]:
        result["suggestions"].append("缺少友商相关新闻")
    if not result["has_material_news"]:
        result["suggestions"].append("缺少原材料价格新闻")
    if result["total_count"] < 10:
        result["suggestions"].append(f"新闻总量偏少（{result['total_count']}条）")
    
    return result


def get_supplier_analysis_prompt(
    supplier_name: str,
    today: str,
    material_categories: list,
    related_companies: list,
    news_summary: str,
    news_count: int
) -> str:
    """生成供应商分析报告的提示词"""
    return f"""# 分析任务
为 **{supplier_name}** 生成供应商分析报告。

**日期**：{today}
**分析视角**：以立讯技术为客户

# 供应商信息
- 名称：{supplier_name}
- 供应物料：{', '.join(material_categories)}
- 主要客户：{', '.join(related_companies)}

# 相关新闻（{news_count}条）
{news_summary if news_summary else '⚠️ 暂无相关新闻'}

---

# 报告结构

## 一、供应商概况
简要介绍（2-3句话）

## 二、近期动态
有新闻就分析，没有就跳过

## 三、供应能力评估
| 维度 | 评分 | 说明 |
|------|------|------|
| 产能 | /10 | |
| 质量 | /10 | |
| 价格 | /10 | |

## 四、风险与建议
具体的风险点和可执行的建议

---
报告长度：500-800字
"""

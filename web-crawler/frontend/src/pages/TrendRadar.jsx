...
const TrendRadar = () => {
    ...
    const loadNews = useCallback(async (category, forceRefresh = false) => {
        // 1. 如果不是强制刷新，且缓存中存在有效数据，优先使用缓存回显
        if (!forceRefresh && dataCache.current[category]) {
            const cached = dataCache.current[category];
            // 简单的缓存有效期检查（例如 5 分钟），如果需要的话。
            // 目前用户要求“暂存”，假设只要没刷新就一直有效
            console.log(`[Cache Hit] 使用缓存数据回显: ${category}`);
            if (isMountedRef.current) {

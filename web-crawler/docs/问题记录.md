...
## 8. 类型注解 List 参数过多导致 TypeError

- 标题：类型注解 List 参数过多导致 TypeError
- 时间：2025-12-25
- 模块/接口：`web/web-crawler/prompts/analysis_prompts_v4.py`
- 环境与 Nacos 配置：Python 3.13；N/A
- 现象：调用 `generate_analysis_v4` 接口时报错 `TypeError: Too many arguments for typing.List; actual 2, expected 1`
- 复现步骤：
  - 运行 `analysis_prompts_v4.py` 中 `get_price_with_dates` 函数所在的调用链路
  - 触发类型检查或运行时加载
- 根因分类：代码错误（类型注解语法错误）
  - `List[(str, float)]` 写法使得 `typing.List` 接收到了一个元组 `(str, float)` 导致参数数量判定异常，应明确使用 `Tuple`
- 修复方案：
  - 引入 `Tuple`：`from typing import Tuple`
  - 修正注解：将 `List[(str, float)]` 改为 `List[Tuple[str, float]]`
- 影响评估：
  - 正向：修复运行时错误，API 可正常调用
  - 风险：无
- 源码位置与提交号：
  - `web/web-crawler/prompts/analysis_prompts_v4.py:613`（提交号：N/A）
- 后续防线：
  - 加强静态代码分析（类型检查）
  - 提交前运行 `python -m compileall` 确保语法无误

## 9. Dashboard 汇率重复换算问题
